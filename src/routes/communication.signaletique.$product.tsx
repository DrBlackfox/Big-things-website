...
export const Route = createFileRoute("/publicite/signaletique/$product")({
  loader: ({ params }) => {
    const product = signaletiqueProducts.find((p) => p.slug === params.product);
  ...
  head: ({ params, loaderData }) => {
    ...
    const url = abs(`/publicite/signaletique/${params.product}`);
    return {
      meta: [
        { title },
  ...
  notFoundComponent: SignaletiqueProductNotFound,
  component: SignaletiqueProductPage,
});

function SignaletiqueProductNotFound() {
  const t = useT();
  return (
    <PageShell>
...
function SignaletiqueProductPage() {
